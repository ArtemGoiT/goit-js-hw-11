...
const c=document.querySelector(".gallery");function g(s){const r=s.hits.map(({webformatURL:n,largeImageURL:e,tags:t,likes:o,views:l,comments:u,downloads:d})=>`<li class= "gallery-item">
        ...
        </li>`).join("");c.insertAdjacentHTML("beforeend",r),new m(".gallery a",{captions:!0,captionDelay:250,captionsData:"alt",captionPosition:"bottom"}).refresh()}//! Этот код берет данные, создает разметку для галереи изображений, добавляет ее на веб-страницу и инициализирует галерею с использованием SimpleLightbox.
const h=document.querySelector(".img-search-form"),a=document.querySelector(".loader");h.addEventListener("submit",y);function y(s){s.preventDefault(),c.innerHTML="",a.classList.remove("hidden");const r=s.target.elements.query.value.trim();f(r).then(i=>{i.totalHits===0||r===""?(a.classList.add("hidden"),p.show({message:"Sorry, there are no images matching your search query. Please try again!",messageColor:"white",backgroundColor:"red",position:"topRight"})):(a.classList.add("hidden"),g(i))}),s.target.reset()}//! Обработчик события формы submit (onFormSubmit):
